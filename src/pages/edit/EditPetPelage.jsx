...
export const EditPetPelage = ({ updatePelage = null }) => {
  const [petPelage, setPetPelage] = useState(updatePelage || { name: "" });
  ...
  const handleSubmit = (e) => {
    ...
    if (!lib.validateNonEmpty("name", petPelage.name, setErrors, refs.name))
      return;
    ...
    const save = async () => {
      ...
      try {
        const response = await axios.post(
          `${import.meta.env.VITE_API_URL}pelages`,
          petPelage,
